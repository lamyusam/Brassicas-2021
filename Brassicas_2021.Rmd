...
phenodata.gen2.clean.compcases %>% 
...
for(i in 1:length(measure.vars)){
  ...
  if(i==1){brassica.coefs = data.frame(row.names = row.names(foo))}
  #cbind coefs to df
...
#plot
ggplot(data = brassica.coefs.fdr, aes(x = variable, y = rowname, fill = value)) +
  geom_tile(color = "grey50", size = 0.5) +
  geom_text(aes(label=round(value,4)), size = 5, face = "bold") +
