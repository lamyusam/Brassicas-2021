...
#there's a cluster of samples (all caudatus) that are kind of outlying on PC2. What effect does it have if we remove these?
caud.outliers = subset(ggpcadata, PC2<(-100))$sample
metadata.raph = subset(metadata.raph, !(sample %in% caud.outliers))
raph.gene.counts.clean = raph.gene.counts.clean[,as.character(metadata.raph$sample)]
...
write.csv(raph.wilds.output, file = "Analysis/RNAseq/Tables/raph_wilds_summary_caudatus_outliers_removed.csv")
