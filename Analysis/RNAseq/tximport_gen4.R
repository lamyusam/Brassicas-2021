...
salmfiles.raph = list.files("Data/RNAseq/raph_quant_gen4", full.names = T)
...
nextflowsheet.raph = read.csv("/home/benjamin/Documents/Brassicas_repo/Data/RNAseq/nextflow_samplesheet_raph_gen4.csv") %>%
  mutate(sampleID = stringr::str_match(fastq_1,"RN[0-9]+")[,1],
...
#save
write.csv(txi.raph.counts,file = "Data/RNAseq/raph.gene.counts.gen4.csv")
...
#now we need a list of Salmon transcript output files
salmfiles.brapa = list.files("Data/RNAseq/brapa_quant_gen4/", full.names = T)
...
#convert nextflow labels to original metadata labels
nextflowsheet.brapa = read.csv("/home/benjamin/Documents/Brassicas_repo/Data/RNAseq/nextflow_samplesheet_brass_gen4.csv") %>%
  mutate(sampleID = stringr::str_match(fastq_1,"RN[0-9]+")[,1],
         replicate = paste0("R",replicate))
colnames(txi.brapa.counts) = nextflowsheet.brapa$sampleID[match(colnames(txi.brapa.counts),nextflowsheet.brapa$replicate)]

#save
write.csv(txi.brapa.counts,file = "Data/RNAseq/brapa.gene.counts.gen4.csv")
