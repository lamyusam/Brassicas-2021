...
## Work package 3: Stuff that didn't get done in 2021

One outstanding question is the degree to which genes that are DE between domesticates and wild progenitors show higher expression plasticity in those progenitors. Let's test this. 

First, let's get the set of genes that are DEG with treatment just in the B. rapa wild progenitor:

```{r deseq-treatment-brapa-progenitor-only}
### get DEGs for treatment just in wilds
#subset to remove just brassica rapa wild progenitor 
metadata.brass.subset = subset(metadata.brass.clean, species == "Brassica rapa" & domesticated == "Wild")
brass.gene.counts.clean.subset = brass.gene.counts.clean[,as.character(metadata.brass.subset$sample)]
#check that metadata and count matrices conform
brass.gene.counts.clean.subset = brass.gene.counts.clean.subset[,as.character(metadata.brass.subset$sample)]
#now run model
...
# Run the default analysis for DESeq2
#NA p-values are generated by 0 counts and outliers calculated by Cook's distance.
dds.gene.deg.brass = DESeq(dds.gene, fitType = "parametric", betaPrior = FALSE)
#save for later
save(dds.gene.deg.brass, file = "Analysis/RNAseq/Tables/brassica_rapa_wild_deseq.R")

#results: wheat vs control
degs.brass.rapawild.treatment = results(dds.gene.deg.brass, 
                               name="treatment_Control_vs_Wheat",     
                               alpha = 0.05,
                               lfcThreshold = log2(1))
degs.brass.rapawild.treatment.ids = rownames(subset(degs.brass.rapawild.treatment, padj<=0.05))
#length(degs.brass.rapawild.treatment.ids)
#this is no good- only three genes are responsive to treatment in the wild progenitor
```

Bad news- it seems only three genes are DE with treatment in the wild progenitor, which suggests we may not find much variation fold changes for this comparison. Still, we can try testing whether genes DE between progenitor and cultivar brapa display higher LFC in this treatment comparison on average:

```{r}
# genes that are DE with wilds = degs.brass.cultivated.ids
prog.treat.df = data.frame(DE = row.names(degs.brass.rapawild.treatment) %in% degs.brass.cultivated.ids,
                           prog_treat_response = degs.brass.rapawild.treatment$log2FoldChange)
#looks good
ggplot(prog.treat.df, aes(x = DE, y = prog_treat_response))+
  geom_point()
#yes, mean log2FC is much higher for the genes that are DE between wild and cultivated
t.test(prog_treat_response~DE,data = prog.treat.df)
```

Yes, although the effect size is small and the p-value is borderline (~0.03).

Let's see if the same is true for Raphanus:
...
### get DEGs for treatment just in wilds
#subset to remove just brassica rapa wild progenitor 
metadata.raph.subset = subset(metadata.raph, domesticated == "Wild")
...
# Run the default analysis for DESeq2
#NA p-values are generated by 0 counts and outliers calculated by Cook's distance.
dds.gene.deg.raph = DESeq(dds.gene, fitType = "parametric", betaPrior = FALSE)
#save for later
save(dds.gene.deg.raph, file = "Analysis/RNAseq/Tables/raphanus_progenitor_treatment_deseq.R")

#results: wheat vs control
degs.raph.progenitor.treatment = results(dds.gene.deg.raph, 
                               name="treatment_Control_vs_Wheat",     
                               alpha = 0.05,
                               lfcThreshold = log2(1))
degs.raph.progenitor.treatment.ids = rownames(subset(degs.raph.progenitor.treatment, padj<=0.05))
#length(degs.raph.progenitor.treatment.ids)
#this is a bit better- 41 genes are responsive to treatment in the wild progenitor
```

41 genes are responsive to treatment in the wild progenitor for radish. Now let's ask whether DEGs between progenitor and domesticate have higher plasticity in this comparison than expected:

```{r}
# genes that are DE with wilds = degs.raph.cultivated.ids
prog.treat.df = data.frame(DE = row.names(degs.raph.progenitor.treatment) %in% degs.raph.cultivated.ids,
                           prog_treat_response = degs.raph.progenitor.treatment$log2FoldChange)
#looks good
ggplot(prog.treat.df, aes(x = DE, y = prog_treat_response))+
  geom_point()
t.test(prog_treat_response~DE,data = prog.treat.df)
...
Yes, it looks like log2FC with treatment is much higher for the genes that are DE between wild and cultivated! This effect seems to be much stronger than in Brassica. 
